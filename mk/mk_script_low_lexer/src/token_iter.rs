use crate::{Cursor, Token, TokenKind, TokenLiteralKind, TokenStrLiteral};
...
fn next(input: &str) -> Token {
    ...
    let kind = match cursor.consume().unwrap() {
        ...
        '0'..='9' => TokenKind::Literal(consume_number(&mut cursor)),
        '#' => {
...
fn consume_number(cursor: &mut Cursor) -> TokenLiteralKind {
    match cursor.first() {
