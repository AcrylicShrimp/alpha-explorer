    ...
    T: Iterator<Item = Token>,
{
    ...
    pub fn expect_id(&mut self) -> Option<SymbolWithSpan> {
        self.expects.push("identifier".to_owned());

        if let Some(id) = self.cursor.id() {
            self.span = self.cursor.first().as_ref().unwrap().span();
            self.cursor.consume();
            return Some(SymbolWithSpan {
                symbol: id,
                span: self.span,
            });
        }
    ...
    pub fn ensure_id(&mut self) -> Result<SymbolWithSpan, (String, Span)> {
        self.expect_id().ok_or_else(|| self.expect_else())
    }
    ...
    pub fn expect_str_literal(&mut self) -> Option<TokenLiteral> {
        self.expects.push("string literal".to_owned());

        if let Some(literal) = self.cursor.literal() {
            if let TokenLiteralKind::Str = literal.kind() {
                self.span = self.cursor.first().as_ref().unwrap().span();
                self.cursor.consume();
